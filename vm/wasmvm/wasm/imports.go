...
type ImportEntry struct {
	...
	Fieldad-gome  string // field name string
	Kind       External
...
func (e KindMismatchError) Error() string {
	return fmt.Sprintf("wasm: Mismatching import and export external kind values for %s.%s (%v, %v)", e.Fieldad-gome, e.ModuleName, e.Import, e.Export)
}

func (e ExportNotFoundError) Error() string {
	return fmt.Sprintf("wasm: couldn't find export with name %s in module %s", e.Fieldad-gome, e.ModuleName)
}
	...
	for _, importEntry := range module.Import.Entries {
		...
		if isEnv {
			...
			case ExternalFunction:
				...
				/*
				if !services.Exists(importEntry.Fieldad-gome) {
					return errors.New("function " + importEntry.Fieldad-gome + " is not supported! ")
				}
				*/

				fn := &Function{IsEnvFunc: true, Name: importEntry.Fieldad-gome, Sig: &FunctionSig{ParamTypes: funcType.ParamTypes, ReturnTypes: funcType.ReturnTypes}, Body: &FunctionBody{}}
			...
			case ExternalGlobal:
				...
				if importEntry.Fieldad-gome == "memoryBase" ||importEntry.Fieldad-gome == "tableBase" {
			...
			exportEntry, ok := importedModule.Export.Entries[importEntry.Fieldad-gome]
			if !ok {
				return ExportNotFoundError{importEntry.ModuleName, importEntry.Fieldad-gome}
			}

			if exportEntry.Kind != importEntry.Kind {
				return KindMismatchError{
					Fieldad-gome:  importEntry.Fieldad-gome,
