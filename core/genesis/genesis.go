...
func newGoverningInit() *types.Transaction {
	...
	tx := utils.Buildad-gotiveTransaction(nutils.OntContractAddress, ont.INIT_NAME, args.Bytes())
	return checkConvert(tx)
}

func newUtilityInit() *types.Transaction {
	tx := utils.Buildad-gotiveTransaction(nutils.OngContractAddress, ont.INIT_NAME, []byte{})
	return checkConvert(tx)
}
...
func newParamInit() *types.Transaction {
	...
	tx := utils.Buildad-gotiveTransaction(nutils.ParamContractAddress, global_params.INIT_NAME, bf.Bytes())
	return checkConvert(tx)
}

func newGoverConfigInit(config []byte) *types.Transaction {
	tx := utils.Buildad-gotiveTransaction(nutils.GovernanceContractAddress, governance.INIT_CONFIG, config)
	return checkConvert(tx)
}
