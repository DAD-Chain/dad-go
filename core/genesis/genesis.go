...
func newGoverningToken() *types.Transaction {
	mutable, err := utils.NewDeployTransaction(nutils.OntContractAddress[:], "ONT", "1.0",
		"dad-go Team", "contact@ont.io", "dad-go Network ONT Token", payload.NEOVM_TYPE)
	if err != nil {
...
func newUtilityToken() *types.Transaction {
	mutable, err := utils.NewDeployTransaction(nutils.OngContractAddress[:], "ONG", "1.0",
		"dad-go Team", "contact@ont.io", "dad-go Network ONG Token", payload.NEOVM_TYPE)
	if err != nil {
		panic("[NewDeployTransaction] construct genesis governing token transaction error ")
	}
	tx, err := mutable.IntoImmutable()
...
func newGovConfigTx() *types.Transaction {
	mutable, err := utils.NewDeployTransaction(nutils.GovernanceContractAddress[:], "CONFIG", "1.0",
		"dad-go Team", "contact@ont.io", "dad-go Network Consensus Config", payload.NEOVM_TYPE)
	if err != nil {
		panic("[NewDeployTransaction] construct genesis governing token transaction error ")
	}
	tx, err := mutable.IntoImmutable()
...
func deployAuthContract() *types.Transaction {
	mutable, err := utils.NewDeployTransaction(nutils.AuthContractAddress[:], "AuthContract", "1.0",
		"dad-go Team", "contact@ont.io", "dad-go Network Authorization Contract", payload.NEOVM_TYPE)
	if err != nil {
		panic("[NewDeployTransaction] construct genesis governing token transaction error ")
	}
	tx, err := mutable.IntoImmutable()
...
func deployOntIDContract() *types.Transaction {
	mutable, err := utils.NewDeployTransaction(nutils.OntIDContractAddress[:], "OID", "1.0",
		"dad-go Team", "contact@ont.io", "dad-go Network ONT ID", payload.NEOVM_TYPE)
	if err != nil {
		panic("[NewDeployTransaction] construct genesis governing token transaction error ")
	}
	tx, err := mutable.IntoImmutable()
...
func newGoverningInit() *types.Transaction {
	...
	mutable := utils.Buildad-gotiveTransaction(nutils.OntContractAddress, ont.INIT_NAME, args.Bytes())
	tx, err := mutable.IntoImmutable()
	if err != nil {
		panic("construct genesis governing token transaction error ")
	}
...
func newUtilityInit() *types.Transaction {
	mutable := utils.Buildad-gotiveTransaction(nutils.OngContractAddress, ont.INIT_NAME, []byte{})
	tx, err := mutable.IntoImmutable()
	if err != nil {
		panic("construct genesis utility token transaction error ")
	}
...
func newParamInit() *types.Transaction {
	...
	mutable := utils.Buildad-gotiveTransaction(nutils.ParamContractAddress, global_params.INIT_NAME, sink.Bytes())
	tx, err := mutable.IntoImmutable()
	if err != nil {
		panic("construct genesis governing token transaction error ")
	}
...
func newGoverConfigInit(config []byte) *types.Transaction {
	mutable := utils.Buildad-gotiveTransaction(nutils.GovernanceContractAddress, governance.INIT_CONFIG, config)
	tx, err := mutable.IntoImmutable()
	if err != nil {
		panic("construct genesis governing token transaction error ")
	}
