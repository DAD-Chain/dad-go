...
var (
	...
	ConfigFlag = cli.StringFlag{
		Name:  "config",
		Usage: "Use `<filename>` to specifies the genesis block config file. If doesn't specifies the genesis block config, dad-go will use Polaris config with VBFT consensus as default.",
	}
	...
	DisableEventLogFlag = cli.BoolFlag{
		...
		Usage: "If set disableeventlog flag, dad-go will not record event log output by smart contract",
	}
	...
	EnableTestModeFlag = cli.BoolFlag{
		Name:  "testmode",
		Usage: "Using to start a single node test network for ease of development and debug. In testmode, dad-go will start rpc, rest and web socket server, and set default gasprice to 0",
	}
	...
	NetworkIdFlag = cli.UintFlag{
		Name:  "networkid",
		Usage: "Using to specify the network ID. Different networkids cannot connect to the blockchain network. 1=dad-go main net, 2=polaris test net, 3=testmode, and other for custom network",
	...
	RPCDisabledFlag = cli.BoolFlag{
		Name:  "disablerpc",
		Usage: "Using to shut down the rpc server. The dad-go node starts the rpc server by default at startup.",
	}
	...
	AccountPassFlag = cli.StringFlag{
		Name:   "password,p",
		Hidden: true,
		Usage:  "Using to specify the account `<password>` when dad-go node starts. Because the account password entered in the command line is saved in the log, it is easy to leak the password. Therefore, it is not recommended to use this parameter in a production environment.",
	}
	AccountAddressFlag = cli.StringFlag{
		Name:  "account,a",
		Usage: "Using to specify the account `<address|label|index>` when the dad-go node starts. If the account is null, it uses the wallet default account",
	}
